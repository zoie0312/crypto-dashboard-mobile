...
const apiKey =
    "EO0m8AeCWdXWsaK0faFFvLGBPwo8ie2B3Sw78W3stNYOVKkEvtL2O90ISog80p9s";
...
const useWallet = ({
    ...
    const fetchData = useCallback(() => {
        ...
        const doFetchData = async () => {
            ...
            try {
                ...
                const [result, nativeResult] = await Promise.all([
                    axios(balanceConfig),
                    axios(nativeBalanceConfig),
                ]);
                //console.log('fetch tokens result ', result);
                const filterResult = result.data.filter((token) => {
                    return (
                        SupportTokens[token.symbol] &&
                        SupportTokens[token.symbol]["token_address"] ===
                            token["token_address"]
                    );
                });
                let tokenAsset = filterResult.map((tokenData) => {
                    const { symbol, thumbnail, balance, decimals } = tokenData;
                    return {
                        symbol,
                        thumbnail,
        ...
        doFetchData();
    }, [address, chain, cryptoPrices, portfolioDispatch]);
