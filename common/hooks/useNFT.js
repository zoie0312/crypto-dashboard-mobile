...
const fetchFloorPrices = (contractAddresses) => {
    const fetchPriceTasks = contractAddresses.map((address) => {
    ...
    const fetchData = useCallback(() => {
        ...
        const doFetchData = async () => {
            ...
            try {
                ...
                let nftAddresses =
                    result.ownedNfts &&
                    result.ownedNfts.reduce((acc, item) => {
                        if (!item.error) {
                ...
                const nftPrices = priceData.reduce((acc, item) => {
                    ...
                    result.floorPriceInUSD =
                        result.floorPrice *
                        cryptoPrices[result.priceCurrency]["USD"];
                    acc.push(result);
                    return acc;
                }, []);
        ...
        doFetchData();
    }, [address, chain, cryptoPrices, portfolioDispatch, nftPriceDispatch]); //don't need to add updateNFT as dependency as it's from PortfolioContext
