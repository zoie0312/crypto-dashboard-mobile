...
const reducer = (state, action) => {
    ...
    const targetWallet = nextState.wallets.find(
    ...
    switch (action.type) {
        ...
        case "REMOVE_ADDRESS":
            nextState.wallets = nextState.wallets.filter(
                (wallet) =>
                    wallet.chain !== action.payload.chain ||
                    ...
                    wallet.nftAssets.forEach((nft) => {
                        if (wallet.chain === nftChain) {
                            const updatePriceData =
                                addressPriceMap[nft.contractAddress];
                            if (updatePriceData) {
                                const {
                                    floorPrice,
                                    priceCurrency,
